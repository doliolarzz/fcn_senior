...
class MRUNet(nn.Module):
    ...
    def __init__(self, config, state_weight=0.7, geo_size=4):
        ...
        self.geo = nn.Parameter(data=torch.randn(geo_size, self.h, self.w), requires_grad=True).to(self.config['DEVICE'])
    ...
    def get_optFlow(self, input):
        ...
        for b in range(input.shape[0]):
            for i in range(input.shape[1] - 1):
                ...
                opt[b, i*2] = delta[...,0]
                opt[b, i*2+1] = delta[...,1]
        return torch.autograd.Variable(data=torch.from_numpy(opt).float(), requires_grad=False).to(self.config['DEVICE'])
